...
unsigned int CBootloader::getLastAddr() {
	...
	if (nBytes== 2) {
		fprintf(stderr, "Warning: device not reporting last address.\n", nBytes);
		return 0xFFFF;
	}
...
int CBootloader::writePage(CPage* page) {
	int nBytes;
	int retries = 5;
	do {
		if (verifyPage(page,false) == +1) { return +1; }
		//write page
		nBytes = usb_control_msg(usbhandle,
		...
		if (nBytes == page->getPagesize()) return verifyPage(page,true);
		usleep(15000);
...
int CBootloader::verifyPage(CPage* page, bool errorOutput) {
	unsigned char buffer[1024]; //need only 64 bytes or 32 bytes really
	char debug_buf[1024];

	if (haveVerifyCommand) {
		int nBytes;
		int retries = 30;
		do {
			nBytes = usb_control_msg(usbhandle,
