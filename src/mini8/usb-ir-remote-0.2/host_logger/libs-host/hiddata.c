...
HANDLE                              resultHandle[3] = {INVALID_HANDLE_VALUE,INVALID_HANDLE_VALUE,INVALID_HANDLE_VALUE};
HIDD_ATTRIBUTES                     deviceAttributes;
				
    HidD_GetHidGuid(&hidGuid);
    deviceInfoList = SetupDiGetClassDevs(&hidGuid, NULL, NULL, DIGCF_PRESENT | DIGCF_INTERFACEDEVICE);
    deviceInfo.cbSize = sizeof(deviceInfo);
    int totalDevicesMatch = 0;
    for(i=0;;i++){
        ...
        resultHandle[totalDevicesMatch] = handle;
        totalDevicesMatch++;
        break;  /* we have found the device we are looking for! */
    }
    if (totalDevicesMatch >= 1) {
    	printf("matches:%d",totalDevicesMatch);
    }
    SetupDiDestroyDeviceInfoList(deviceInfoList);
    if(deviceDetails != NULL)
        free(deviceDetails);
    if(resultHandle[0] != INVALID_HANDLE_VALUE){
        *device = (usbDevice_t *)resultHandle[0];
        errorCode = 0;
    }
    return errorCode;
}
