...
/* Christian Unhold: Adapted for the mini8 board
 * Enter bootloader when key1 is pressed, exit when key2 is pressed
 * Indicate active bootloader by three weak glowing dots on the dotmatrix
 */
...
{
	PORTC = 1<<PC4|1<<PC3; /* activate pull-ups on keys */
	DDRB  = 1<<PB5|1<<PB4|1<<PB3; /* enable row outputs */
	PORTB = 0<<PB5|1<<PB4|1<<PB3; /* set row to 4 */
	PORTD = 1<<PD7|1<<PD6|1<<PD5; /* drive col 2, 3 and 4 thru pull-up only */
}

static inline void  bootLoaderExit(void)
{
	PORTD = 0; /* undo bootLoaderInit() and usbInit() changes */
	DDRD  = 0;
	PORTB = 0;
	DDRB  = 0;
	PORTC = 0;
...
#define bootLoaderEnterCondition() (!(PINC&(1<<PC4)))

#define bootLoaderLeaveCondition() (!(PINC&(1<<PC3)))
