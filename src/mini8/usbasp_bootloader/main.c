...
/* Christian Unhold: Fine tuned to save a couple of flash words
 * Remove wdt_reset() in delay loop of initForUsbConnectivity():
 *      watchdog is disabled in before, in main()
 * Declare requestBootLoaderExit only #if BOOTLOADER_CAN_EXIT:
 *      else it is not used
 * Add attribute noreturn to main because main can never return
 */
...
static inline uchar usbFunctionSetup(uchar data[8])
{
...
static inline uchar usbFunctionWrite(uchar *data, uchar len)
{
uchar   isLast;
...
static inline uchar usbFunctionRead(uchar *data, uchar len)
{
uchar   i;
...
static void initForUsbConnectivity(void)
{
    ...
    _delay_ms(256); /* fake USB disconnect for > 250 ms */
    usbDeviceConnect();
    sei();
}
...
#endif
        }while(!bootLoaderLeaveCondition());  /* main event loop */
    }
    leaveBootloader();
}
